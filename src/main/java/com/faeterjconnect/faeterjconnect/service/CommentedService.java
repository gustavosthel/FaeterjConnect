...
public class CommentedService {
    ...
    public PageResponse<CommentViewDTO> listComments(UUID postId, int page, int size) {
        ...
        Sort sort = Sort.by(Sort.Order.asc("commentTime"), Sort.Order.asc("commentId"));
        Pageable pageable = PageRequest.of(page, size, sort);

        Page<CommentedEntity> cPage = commentedRepository.findByPost_PostId(postId, pageable);
        ...
        return new PageResponse<>(
                items,
                cPage.getNumber(),        // número da página retornada
                cPage.getSize(),          // tamanho da página retornada
                cPage.getTotalElements(), // total de elementos
                cPage.getTotalPages(),    // total de páginas
                cPage.hasNext(),          // se existe próxima página
                cPage.hasPrevious()       // se existe página anterior
        );
    }
